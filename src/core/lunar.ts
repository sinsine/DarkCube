/** 农历换算（1900–2100）+ 中国新年彩蛋判断 */

// 标准农历数据表：每年一个 16 进制数
// 位 0-3：闰月月份（0 为无闰月）；位 4-15：各月天数（1=30 天，0=29 天）；0x10000：闰月为 30 天
const lunarInfo = [
...
/** 是否处于中国新年彩蛋期：农历除夕（廿九/三十）至正月初七 */
export function isCnyPeriod(date = new Date()): boolean {
  const l = solarToLunar(date.getFullYear(), date.getMonth() + 1, date.getDate())
  if (l.month === 12 && l.day >= 29) return true // 除夕
  if (l.month === 1 && l.day <= 7) return true // 正月初一至初七
  return false
}
